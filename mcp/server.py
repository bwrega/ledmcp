...
url_prefix = "http://192.168.0.101:7777"
...
def turn_on_led() -> str:
    """Turns on the LED"""
    response = requests.get(f"{url_prefix}/led-on")
...
def turn_off_led() -> str:
    """Turns off the LED"""
    response = requests.get(f"{url_prefix}/led-off")
    if response.status_code == 200:
...
def send_morse_message(text: str) -> str:
    ...
    response = requests.get(f"{url_prefix}/morse", params={"text": text})
    if response.status_code == 200:
